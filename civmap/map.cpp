...
namespace civ
{
    namespace V2I
    {
        ...
        double CivMap::get_distance_map_llh(const Eigen::Vector3d &pt_llh, Eigen::Vector3d &cross_pt_map_llh)
        {
            Eigen::Vector3d pt_enu;
            Eigen::Vector3d cross_pt_map_enu;
            using namespace civ::common::coord_transform;
            ConvertLLAToENU(Earth::GetOrigin(), pt_llh, &pt_enu);
            double distance = get_distance_map_enu(pt_enu, cross_pt_map_enu);
            ConvertENUToLLA(Earth::GetOrigin(), cross_pt_map_enu, &cross_pt_map_llh);
            return distance;
        }

        double CivMap::get_distance_map_enu(const Eigen::Vector3d &pt_enu, Eigen::Vector3d &cross_pt_map_enu)
        {
            ...
            for (const auto &curve_enu : lines_enu)
            {
                ...
                double distance = get_distance_curve_enu(pt_enu, curve_enu, cross_pt_curve_enu);
        ...
        double CivMap::get_distance_curve_enu(const Eigen::Vector3d &pt_enu, sp_cZMapLineSegment curve, Eigen::Vector3d &cross_pt_curve)
        {
            using namespace civ::common;
