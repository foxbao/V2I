...
class Earth {
 ...
 public:
  ...
  static Eigen::Matrix3d Pos2Cne(const Eigen::Vector3d &pos,
    ...
    Eigen::Matrix3d re = Eigen::Matrix3d(
        Eigen::AngleAxisd(-(M_PI / 2 - b), Eigen::Vector3d::UnitX()) *
        Eigen::AngleAxisd(-(M_PI / 2 + l), Eigen::Vector3d::UnitZ()));
    return re;
  }
 ...
 private:
  ...
  static double M(const double B_) {
...
template <typename T>
inline Eigen::Matrix<T, 3, 3> EulerToMatrix(
    Eigen::Matrix<T, 3, 1> const &euler) {
