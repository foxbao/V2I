...
CubicSpline::CubicSpline(void** buf)
{
	assert(nullptr != buf);
	void* dsbuf = *buf;
	...
	for (uint64_t i = 0; i < *sz; i++) {
        ...
        dsbuf = (void*)(s0 + 1);
		Poly3 p = Poly3(&dsbuf);
		s0_to_poly.insert(std::map<double, Poly3>::value_type(*s0, p));
	}
    *buf = dsbuf;
}

uint64_t CubicSpline::getsize()
{
	uint64_t sz = sizeof(uint64_t);
	std::map<double, Poly3>::iterator it;
    for(it = s0_to_poly.begin(); it != s0_to_poly.end(); ++it) {
        sz += sizeof(double);   
...
void* CubicSpline::deserialize(void* dsbuf)
{
    assert(nullptr != dsbuf);
	uint64_t* sz = (uint64_t*)dsbuf;
	dsbuf = (void*)(sz + 1);
...
void* CubicSpline::serialize(void* buf)
{
	uint64_t* number = (uint64_t*)buf;
	*number = s0_to_poly.size();
	buf = (void*)(number + 1);
	std::map<double, Poly3>::iterator it;
	for(it = s0_to_poly.begin(); it != s0_to_poly.end(); ++it) {
		double* s0 = (double*)buf;
		*s0 = it->first;
		buf = it->second.serialize((void*)(s0 + 1));
	}
	return buf;
}

Poly3::Poly3(void **buf)
{
	assert(nullptr != buf);
	void* dsbuf = *buf;
	assert(nullptr != dsbuf);
	double* pa = (double*)dsbuf;
	a = *pa;
	dsbuf = (void*)(pa+1);
	double* pb = (double*)dsbuf;
	b = *pb;
	dsbuf = (void*)(pb+1);
	double* pc = (double*)dsbuf;
	c = *pc;
	dsbuf = (void*)(pc+1);
	double* pd = (double*)dsbuf;
	d = *pd;
	dsbuf = (void*)(pd+1);
...
void* Poly3::serialize(void* buf)
{
	double* pa = (double*)buf;
	*pa = a;
	buf = (void*)(pa+1);
	double* pb = (double*)buf;
	*pb = b;
	buf = (void*)(pb+1);
	double* pc = (double*)buf;
	*pc = c;
	buf = (void*)(pc+1);
