...
struct LanesMesh : public RoadsMesh
{
    ...
    std::map<size_t, std::string>    lane_start_type;
};
...
struct RoadObjectsMesh : public RoadsMesh
{
    ...
    std::array<size_t, 2> get_idx_interval_road_object_type(size_t vert_idx) const;

    std::map<size_t, std::string> road_object_start_indices;
    std::map<size_t, std::string> road_object_type;
};

struct RoadNetworkMesh
{
    Mesh3D get_mesh() const;
    double get_road_height(double x, double y, double distance = 25.0);
