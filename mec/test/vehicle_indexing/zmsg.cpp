#include "zmsg.h"

#include <zmq.hpp> 
#include <iostream>
#include <iomanip>
#include <sstream>

#include <assert.h>
#include <stdio.h>
#include <stdarg.h>
#include <signal.h>

#include <unistd.h>
#include <vector>
#include <string>

#include "webcore/logger.h"

namespace zas {
namespace vehicle_indexing
{

using namespace zas::webcore;

void s_version_assert (int want_major, int want_minor)
{
	int major, minor, patch;
	zmq_version (&major, &minor, &patch);
	if (major < want_major
	|| (major == want_major && minor < want_minor)) {
		log.i("zmsg", "current 0MQ version is %d.%d\n", major, minor);
		log.i("zmsg", "application needs at least %d.%d"
			" - cannot continue\n", want_major, want_minor);
		exit (EXIT_FAILURE);
	}
}

zmsg::zmsg()
: _body_index(0)
, _refcnt(1)
{
}

zmsg::zmsg(char const *body, size_t sz)
: _body_index(0)
, _refcnt(1)
{
	body_set(body, sz);
}

zmsg::zmsg(char const *body, size_t sz, zmq::socket_t &socket)
{
	body_set(body, sz);
	send(socket);
}

zmsg::zmsg(zmq::socket_t &socket) 
: _body_index(0)
, _refcnt(1)
{
	recv(socket);
}

zmsg::zmsg(zmsg &msg)
: _body_index(0)
, _refcnt(1)
{
	_part_data.resize(msg._part_data.size());
	std::copy(msg._part_data.begin(), msg._part_data.end(), _part_data.begin());
}

zmsg::~zmsg()
{
	clear();
}

int zmsg::clear()
{
	_part_data.clear();
	return 0;
}

int zmsg::set_part(size_t part_nbr, char *data, size_t sz)
{
	if (part_nbr < _part_data.size()) {
		_part_data[part_nbr].assign(data, sz);
	}
	return sz;
}

bool zmsg::recv(zmq::socket_t & socket)
{
	clear();
	while(1) {
		zmq::message_t message(0);
		try {
			if (!socket.recv(&message, 0)) {
				return false;
			}
		} catch (zmq::error_t error) {
			log.e("zmsg", "E: %s\n", error.what());
			return false;
		}

		_part_data.push_back(
			std::string((char*)message.data(),
			message.size()));
		
		// is empty delimiter frame
		if (message.size() == 0)
			_body_index = _part_data.size();

		if (!message.more())
			break;
	}
	return true;
}

int zmsg::send(zmq::socket_t & socket)
{
	for (size_t part_nbr = 0; part_nbr < _part_data.size(); part_nbr++) {
		zmq::message_t message;
		std::string data = _part_data[part_nbr];
		message.rebuild(data.c_str(), data.size());

		try {
			socket.send(message, 
				part_nbr < _part_data.size() - 1 ? ZMQ_SNDMORE : 0);
		} catch (zmq::error_t error) {
			assert(error.num()!=0);
		}
	}
	return 0;
}

size_t zmsg::parts()
{
	return _part_data.size();
}

size_t zmsg::body_parts()
{
	if (_body_index < 0) {
		return 0;
	}
	if (_part_data.size() < _body_index) {
		return 0;
	}
	return _part_data.size() - _body_index;
}
int zmsg::body_set(const char *body, size_t sz)
{
	if (_part_data.size() == 0 || _body_index == 0) {
		log.e("zmsg", "err: zmsg nobody\n");
		return -ENOTAVAIL;
	}
	if (_part_data.size() > 0) {
		std::vector<std::string>::iterator it = _part_data.begin();
		advance(it, _body_index);
		_part_data.erase(it, _part_data.end());
	}
	push_back((char*)body, sz);
	return 0;
}

int zmsg::body_insert(int index, const char *body, size_t sz)
{
	if (_part_data.size() == 0 || _body_index == 0) {
		log.e("zmsg", "err: zmsg nobody\n");
		return -ENOTAVAIL;
	}
	if (_body_index + index >= _part_data.size()) {
		log.e("zmsg", "err: zmsg invailed index\n");
		return -EBADPARM;
	}

	std::vector<std::string>::iterator it = _part_data.begin();
	advance(it, _body_index);
	_part_data.insert(it, std::string(body, sz));
	return 0;
}

int zmsg::push_body_front(char *part, size_t sz) {
	body_insert(0, part, sz);
	return 0;
}


int zmsg::body_fmt(const char *format, ...)
{
	char value [255 + 1];
	va_list args;

	va_start (args, format);
	vsnprintf (value, 255, format, args);
	va_end (args);

	body_set(value, strlen(value));
	return 0;
}

std::string zmsg::body(int index)
{
	if (_part_data.size() == 0
		|| (_body_index + index) >= _part_data.size()) {
		return nullptr;
	}
	return _part_data[_body_index + index];
}

int zmsg::push_front(char *part, size_t sz) {
	_part_data.insert(_part_data.begin(), std::string(part, sz));
	return 0;
}

int zmsg::push_back(char *part, size_t sz) {
	_part_data.push_back(std::string(part, sz));
	return 0;
}

std::string zmsg::pop_front() {
	if (_part_data.size() == 0) {
		return "";
	}
	std::string part = _part_data.front();
	_part_data.erase(_part_data.begin());
	if (_body_index > 0) {
		_body_index--;
	} 
	return part;
}

int zmsg::append(const char *part, size_t sz)
{
	assert (part);
	push_back((char*)part, sz);
	return 0;
}

char* zmsg::get_front() {
	if (_part_data.size() > 0) {
		return (char*)_part_data[0].c_str();
	} else {
		return nullptr;
	}
}

std::string zmsg::get_part(int index) {
	if (_part_data.size() > index) {
		return _part_data[index];
	} else {
		return nullptr;
	}
}

int zmsg::dump()
{
	for (int i = 0; i < _part_data.size(); i ++) {
		log.d("zmsg", "msgdata %d:	%s\n", i, _part_data[i].c_str());
	}	
	return 0;
}

}}
