...
kdtree2d_search(T* kdtree, double x, double y, double &dist)
{
    ...
    while(s != nullptr) {
		search_path.push(s);
		if (!s->get_split()) {
			s = (T*)((x <= s->x()) ? s->get_left() : s->get_right());
		} else {
			s = (T*)((y <= s->y()) ? s->get_left() : s->get_right());
		}
	}
	...
	while (search_path.size()) {
		...
		if (!back->get_left() && !back->get_right()) {	
			if (d > tmp) {
				nearest = back; d = tmp;
			}
			continue;
		...
		if (split == 0) {
			...
			if (fabs(back->x() - x) < d) {
				...
				s = (T*)((x <= back->x()) ? back->get_right() : back->get_left());
			...
			if (fabs(back->y() - y) < d) {
				...
				// see comments above
				s = (T*)((y <= back->y()) ? back->get_right() : back->get_left());
				if (s) search_path.push(s);
			}
		}
	}
...
template <typename T> static T* do_build_kdtree(T **exm_set, int size)
{
	...
	ret->set_left(do_build_kdtree(exm_set_left, size_left));
	ret->set_right(do_build_kdtree(exm_set_right, size_right));
