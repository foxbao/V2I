 ...
 * definition of java code generation for zrpc
 */

#ifndef __JAVA_RPCC_CODEGENJAVA_H__
#define __JAVA_RPCC_CODEGENJAVA_H__
...
class module_object_java_code_generator : public module
{
	enum {
		MODOBJ_JAVA_CODEGEN_INTERNAL_HDRFILE = 1,
	};
public:
	module_object_java_code_generator(const string& modname,
		grammar_parser* gpsr, config& cfg);
	~module_object_java_code_generator();
...
private:
	...
	void create_srcfile(const char *filename);
	void create_virtual_srcfile(const char* filename);
	void create_enum_srcfile(const char* filename);
	void create_event_srcfile(const char *filename);
	void create_final_srcfile(const char *filename);
	void create_observer_srcfile(const char *filename);
	void create_observer_interface_srcfile(const char *filename);
	...
	// file operations
	bool fprintf_common(const char* filename, srcfile_type type, 
		const char* content, va_list ap);
	FILE** getfile(const char* filename, srcfile_type type);
...
private:
	...
	FILE* _fp_module_source_virtual;
	FILE* _fp_module_enum_source;
	FILE* _fp_module_event_source;
	FILE* _fp_module_final_source;
	FILE* _fp_module_observer_source;
	FILE* _fp_module_observer_interface_source;
...
class typedef_object_java_code_generator : public typedef_object
{
public:
	typedef_object_java_code_generator(module* mod,
	...
	~typedef_object_java_code_generator();

	bool generate_code(void);
};

class constitem_object_java_code_generator : public TConstItemObject
{
public:
	constitem_object_java_code_generator(module* mod,
		const string& name, number_type type, double& value);
	~constitem_object_java_code_generator();

	bool generate_code(void);
};

class enumdef_object_java_code_generator : public enumdef_object
{
public:
	enumdef_object_java_code_generator(module *mod,
		const string name);
	~enumdef_object_java_code_generator();

	bool generate_code(void);
};

class userdeftype_object_java_code_generator : public userdef_type_object
{
public:
	userdeftype_object_java_code_generator(module* mod,
		const string& name);
...
class interface_object_java_code_generator : public interface_object
{
public:
	interface_object_java_code_generator(module* mod, const string& name);
	~interface_object_java_code_generator();
...
private:
	...
	bool has_observer_param(bool root,TInterfaceNode* node, TVariableObject *varObj);
	bool has_construct();
	bool has_virtual();
...
private:
	...
	int generate_attribute_body(const char* clsname, TInterfaceNode* node);
	int attribute_variable_name(module* cmod, TInterfaceNode* node, 
		string& ret);
	int	get_attribute_full_name(module* cmod, TVariableObject* var, 
		string& ret);
	...
	int variable_full_name(module* cmod, TVariableObject* var,
		string& ret, bool out, bool proxy,
		bool observable = false, bool bret = false);
	int variable_define_full_name(module* cmod, TVariableObject* var,
		string& ret, bool proxy,
		bool observable = false, bool brefs = false);
	int method_retval_variable_name(module* cmod, TInterfaceNode* node,
		string& ret, bool out, bool proxy, bool observable = false);
	int generate_uid(const char* name, std::string& uid);
};

class event_object_java_code_generator : public TEventObject
{
public:
	event_object_java_code_generator(module *mod,
		const string name);
	~event_object_java_code_generator();
...
#endif /* __JAVA_RPCC_CODEGENJAVA_H__ */
/* EOF */
